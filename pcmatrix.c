...
int main (int argc, char * argv[])
{
  ...
  bigmatrix = (Matrix **) malloc (sizeof (Matrix *) * BOUNDED_BUFFER_SIZE);
  ProdConsStats* pStats = malloc (sizeof (ProdConsStats)* BOUNDED_BUFFER_SIZE);
  ProdConsStats* cStats = malloc (sizeof (ProdConsStats)* BOUNDED_BUFFER_SIZE);
  for (int i = 0; i < numw; i++) {
  	pthread_create(&pr[i], NULL, prod_worker, pStats);
    pthread_create(&co[i], NULL, cons_worker, cStats);
  }
  ...
  printf("Sum of Matrix elements --> Produced=%d = Consumed=%d\n",pStats->sumtotal,cStats->sumtotal);
  printf("Matrices produced=%d consumed=%d multiplied=%d\n",pStats->matrixtotal,cStats->matrixtotal,cStats->multtotal);
